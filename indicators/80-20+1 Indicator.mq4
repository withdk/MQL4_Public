...
bool flagBear(int i)
  {
   ...
   double CurrHigh = iHigh(Symbol(),0,i);
   double CurrClose = iClose(Symbol(),0,i);

   double UpperPercent=80; // open and close in upper/lower 20%
   double LowerPercent=20;

   double PrevOpen = iOpen(Symbol(),0,i+1);
   double PrevHigh = iHigh(Symbol(),0,i+1);
   double PrevLow = iLow(Symbol(),0,i+1);
   ...
   //if(PrevRange < (iATR(Symbol(),0,14,i+1+1)*1))
      //return(false);
...
//if(iClose(Symbol(),0,i+1)>MAAlertCurr2)
   dBearMaTouchBuffer[i] = iHigh(Symbol(),0,i) + 5000*Point;
//else
...
bool flagBull(int i)
  {
   ...
   bool bCondition=false;
   double CurrLow = iLow(Symbol(),0,i);
   double CurrClose = iClose(Symbol(),0,i);
   ...
   //if(PrevRange < (iATR(Symbol(),0,14,i+1)*1))
      //return(false);
...
//if(ShowCircles)
//if(iClose(Symbol(),0,i+1)>MAAlertCurr2)
   dBullMaTouchBuffer[i] = iLow(Symbol(),0,i) - 5000*Point;
//else
//dBearMaTouchBuffer[i] = High[i] + 50*Point;
   return(bCondition);
  }
